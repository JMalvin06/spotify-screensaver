                ...
                Ok(parsed) => {
                    ...
                    let file = match fs::read_to_string("users.json"){ 
                        Ok(f) => {
                            ...
                            let cleaned_file: String = f.chars()
                            .filter(|c| !c.is_whitespace())
                            .collect();
                            
                            // Check if users.json does not follow necessary format, and correct
                            if f.is_empty() || (!cleaned_file.contains("{\"users\":[") && !cleaned_file.contains("]}")){
                    ...
                    if user_exists == -1 { // if user does NOT exist, add new json entry
                        println!("Old json: {}", serde_json::to_string_pretty(&file).expect("Oopsie"));
...
pub(crate) fn get_user_list() -> Vec<String>{
    ...
    file.users.iter().map(|u| u.name.clone()).collect() // Parse json file and generate a list of usernames
}
